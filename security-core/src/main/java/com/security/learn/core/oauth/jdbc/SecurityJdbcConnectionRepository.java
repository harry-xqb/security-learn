package com.security.learn.core.oauth.jdbc;
...
/**
 * create by： harry
 * date:  2019/12/1 0001 下午 4:01
 **/
public class SecurityJdbcConnectionRepository implements ConnectionRepository {
    ...
    public SecurityJdbcConnectionRepository(String userId, JdbcTemplate jdbcTemplate, ConnectionFactoryLocator connectionFactoryLocator, TextEncryptor textEncryptor, String tablePrefix) {
        this.userId = userId;
    ...
    public MultiValueMap<String, Connection<?>> findAllConnections() {
        List<Connection<?>> resultList = jdbcTemplate.query(selectFromUserConnection() + " where userId = ? order by providerId, level", connectionMapper, userId);
    ...
    public List<Connection<?>> findConnections(String providerId) {
        return jdbcTemplate.query(selectFromUserConnection() + " where userId = ? and providerId = ? order by level", connectionMapper, userId, providerId);
    }
    ...
    public MultiValueMap<String, Connection<?>> findConnectionsToUsers(MultiValueMap<String, String> providerUsers) {
        ...
        List<Connection<?>> resultList = new NamedParameterJdbcTemplate(jdbcTemplate).query(selectFromUserConnection() + " where userId = :userId and " + providerUsersCriteriaSql + " order by providerId, level", parameters, connectionMapper);
    ...
    public void addConnection(Connection<?> connection) {
        try {
            ...
            int level = jdbcTemplate.queryForObject("select coalesce(max(level) + 1, 1) as level from " + tablePrefix + "UserConnection where userId = ? and providerId = ?", new Object[]{ userId, data.getProviderId() }, Integer.class);
            jdbcTemplate.update("insert into " + tablePrefix + "UserConnection (userId, providerId, providerUserId, level, displayName, profileUrl, imageUrl, accessToken, secret, refreshToken, expireTime) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    userId, data.getProviderId(), data.getProviderUserId(), level, data.getDisplayName(), data.getProfileUrl(), data.getImageUrl(), encrypt(data.getAccessToken()), encrypt(data.getSecret()), encrypt(data.getRefreshToken()), data.getExpireTime());
    ...
    private Connection<?> findPrimaryConnection(String providerId) {
        List<Connection<?>> connections = jdbcTemplate.query(selectFromUserConnection() + " where userId = ? and providerId = ? order by level", connectionMapper, userId, providerId);
    ...
    private final SecurityJdbcConnectionRepository.ServiceProviderConnectionMapper connectionMapper = new SecurityJdbcConnectionRepository.ServiceProviderConnectionMapper();
